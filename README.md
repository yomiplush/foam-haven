...
- Quest 3のハンドトラッキングにも対応。コントローラーを置くと素手に切り替わり、指で膜や風船に触れて押せます。メニューは手が届く位置に寄ってきて、ボタンを指で触って押すだけです。両手を同時にピンチするとメニュー再表示、片手でピンチすると泡、グーで握ると膜を押したり風船をつかめます。
...
- Quest 3のハンドトラッキングに対応。コントローラーを置いて素手になると、指で膜を押したりMRの風船を握ったりできます。メニューは手が届く位置へ寄り、ボタンを指で直接触って押します（指す＋ピンチは不要）。両手同時ピンチでメニューの再表示、片手ピンチで泡を出します。コントローラーを使う時の操作は今まで通りです。
