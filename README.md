...
- Quest 3のハンドトラッキングにも対応。コントローラーを置くと素手の「さわるだけ」モードになり、メニューは出ません。指で膜を押したり、片手で泡を出したり、グーで風船をつかめます。コントローラーを手に取ると通常のメニューと操作に戻ります。
...
- スティック押込み：包まれている自分の姿を「外から見る展示ビュー」に切り替え。もう一度押すと主観に戻ります。
...
- Quest 3のハンドトラッキングに対応。コントローラーを置いて素手になると、メニューは出ず「触って遊ぶだけ」のモードになります。指で膜やMRの風船に触れて押したり握ったり、片手で泡を出せます。景色・設定の変更はコントローラーを手に取れば通常メニューから行えます。
- MR「お部屋に風船」でも、メニューを閉じると最初から二重の泡(膜)に包まれます。膜はメニューからOFFにできます。
- 女の子座りの膝の位置を微調整し、股を少し外側に開いた自然なシルエットにしました。
- スティック押込みで、前方に「泡に包まれた自分の姿」を展示する外からの視点へ切り替えられるようになりました。もう一度押すと主観視点へ戻ります。この間は膜への操作や手の追従はお休みします。
